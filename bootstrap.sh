...
function restore_sealed_secrets_key() {
  echo "Checking for Sealed Secrets Master Key backup..."
  if [ -f "bootstrap/master-key.yaml" ]; then
    echo "Restoring Sealed Secrets Master Key for Disaster Recovery..."
    kubectl apply -f bootstrap/master-key.yaml
  else
    echo "Notice: No master-key.yaml found in bootstrap/ directory. A new key will be generated by the controller."
  fi
}
...
ensure_kind_cluster
restore_sealed_secrets_key  # Inject the master key before GitOps syncs
install_argocd
apply_root_app
print_next_steps